...
public class RosProtocolFactory extends ProtocolImplFactory {
  ...
  public RosProtocolFactory() {
    super("ros", "Provides ROS 1/2 topic bridge support", new NoOpDetection());
  }

  @Override
  public Protocol connect(EndPoint endPoint, String sessionId, String username, String password) throws IOException {
    ExtensionConfigDTO config = (ExtensionConfigDTO) ((ExtensionEndPoint) endPoint).config();
    Protocol protocol = new ExtensionProtocol(endPoint, new RosProtocol(endPoint, config));

    String effectiveSession = sessionId == null || sessionId.isEmpty() ? "ros-session" : sessionId;
    String effectiveUser = username == null || username.isEmpty() ? "ros-client" : username;
    String effectivePassword = password == null ? "" : password;

    protocol.connect(effectiveSession, effectiveUser, effectivePassword);
    return protocol;
  }
